...
import net.kriomant.gortrans.geometry.{closestSegmentPointPortion, Point => Pt}
import net.kriomant.gortrans.parsing.{RoutePoint, RouteStop, VehicleInfo}
    ...
    (2 to points.length - 2).find(i => points(i).stop.isDefined && points(i - 1) == points(i)) match {
      ...
      case None =>
        ...
        val pos = {
          ...
          (1 until stops.length).find(i => stops(i - 1)._1 == stops(i)._1).map(stops(_)._2)
